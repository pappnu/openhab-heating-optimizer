...
public class HeatingOptimizerActionHandlerTest {
    ...
    public void testHeatingContinuation() {
        ...
        var heating = testSuccessfulHeatingOptimization(prices, 3, prices.length, prices.length, 2, prices.length,
                prices.length, 3, true);
    ...
    public void testHeatingPeriods() {
        ...
        var heating = testSuccessfulHeatingOptimization(prices, 4, prices.length, prices.length, 3, prices.length, 4, 2,
                false);
    ...
    protected static double[] testSuccessfulHeatingOptimization(double[] prices, int totalHeatingNeed,
            ...
            boolean previousPeriodEndsInHeating) {
        var result = HeatingOptimizerActionHandler.optimizeHeatingWithLinearProgramming(prices, totalHeatingNeed,
                maxHeatingGapDuringFirstPeriod, maxHeatingGapDuringSecondPeriod, minHeatingPeriod, firstGapSize,
                firstPeriodLength, heatingNeedDuringFirstPeriod, previousPeriodEndsInHeating, SOLVING_TIME_LIMIT);
